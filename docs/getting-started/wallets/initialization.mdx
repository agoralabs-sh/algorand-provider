...
In order for dApps to interact wallets, the dApp will you use the `window.algorand` object.

When injecting a wallet interface, it maybe necessary to create the `window.algorand` object:
...
Next, a wallet manager will need to be created, conforming to the [`BaseWalletManager`](../../api-reference/base-wallet-manager), and can be added to the `window.algorand` provider:
